...
test("installGlobalConfig writes plugin and Honcho commands into global opencode.json", async () => {
  ...
  expect(config.command["honcho:mode"].template).toMatch(/~\/\.honcho\/config\.json/)
  ...
  expect(config.command["honcho:interview"].template).toMatch(/Is there anything Honcho should know about you in particular\?/)
  expect(config.command["honcho:interview"].template).toMatch(/create one durable conclusion from the non-empty response/i)
