...
const resolveHonchoCtor = (sdk) => {
    if (!sdk || typeof sdk !== "object") {
        throw new Error(`Honcho SDK constructor is unavailable. import path=${HONCHO_SDK_IMPORT_PATH}; module keys=${describeSdkShape(sdk)}`);
    }
...
const loadHonchoModule = async (deps = {}) => {
    const requireImpl = deps.requireImpl ?? ((specifier) => requireFromHere(specifier));
    const importImpl = deps.importImpl ?? ((specifier) => import(specifier));
    try {
...
const readVisibleTextPart = (part) => {
...
const upsertAssistantMessagePart = (state, payload) => {
...
const extractCompletedAssistantMessage = (payload, state) => {
